    ...
    try:
        ...
        def textify(message):
            payload = {
                "model": "venice-uncensored",
                "messages": [
                    {"role": "system", "content": textify_sysprompt},
...
def start_inactivity_timer(chat_path):
    ...
    # Start new timer
    wait_time = (
        6000  # count of seconds until LLM response request is triggered for inactivity
    )
...
channel.basic_consume(
    queue="llm_messages", on_message_callback=on_message, auto_ack=False
)
